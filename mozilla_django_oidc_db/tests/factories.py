...
class OIDCProviderFactory(factory.django.DjangoModelFactory):
    ...
    oidc_op_jwks_endpoint = factory.LazyAttribute(
        lambda provider: (
            f"{provider.oidc_op_discovery_endpoint}protocol/openid-connect/certs"
        )
    )
    ...
    oidc_op_token_endpoint = factory.LazyAttribute(
        lambda provider: (
            f"{provider.oidc_op_discovery_endpoint}protocol/openid-connect/token"
        )
    )
    oidc_op_user_endpoint = factory.LazyAttribute(
        lambda provider: (
            f"{provider.oidc_op_discovery_endpoint}protocol/openid-connect/userinfo"
        )
    )
    oidc_op_logout_endpoint = factory.LazyAttribute(
        lambda provider: (
            f"{provider.oidc_op_discovery_endpoint}protocol/openid-connect/logout"
        )
    )
